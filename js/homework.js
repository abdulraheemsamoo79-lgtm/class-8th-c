...
function dueBadge(d) {
  const t = todayStr();
  if (d < t) return `<span class="due late">Due nikal gayi</span>`;
  if (d === t) return `<span class="due soon">Aaj</span>`;
  if (d === addDays(t, 1)) return `<span class="due soon">Kal</span>`;
...
function itemHTML(h, done) {
  ...
  return `<div class="item hw ${done.has(h.id) ? 'done' : ''}" style="--c:${colorOf(h.subject)}">
    <input type="checkbox" data-done="${h.id}" ${done.has(h.id) ? 'checked' : ''} aria-label="Ho gaya">
    <div class="grow">
...
export function renderHomework(view) {
  ...
  view.innerHTML = `
    <div class="toolbar"><div><h2 class="page-title">📝 Homework</h2><p class="page-sub" style="margin:0">Aane wala kaam yahan likha hota hai.</p></div>
      <button class="btn" id="hwAdd">＋ Homework likho</button></div>
    <h3 class="section-title">Aane wala</h3>
    <div class="list" id="hwUp">${up.length ? up.map(h => itemHTML(h, done)).join('') : `<div class="empty"><h3>Koi homework nahi</h3><p>Jab teacher homework den, yahan likh do.</p></div>`}</div>
    ${old.length ? `<h3 class="section-title">Purana</h3><div class="list" id="hwOld">${old.slice(0, 15).map(h => itemHTML(h, done)).join('')}</div>` : ''}`;
  ...
  view.onclick = async e => {
    ...
    if (!await ask({ title: 'Homework delete karein?', ok: 'Delete karo', danger: true })) return;
    try { await deleteHomework(b.dataset.del); toast('Delete ho gaya'); emitChange(); }
...
function openAdd() {
  const t = todayStr();
  const s = sheet(`<h2>Homework likho</h2>
    ...
    <div class="row"><label for="hT">Kya karna hai?</label><input class="field" id="hT" maxlength="100" placeholder="Jaise: Ex 5.2 sawal 1 se 8"></div>
    <div class="row"><label for="hD">Tafseel (zaroori nahi)</label><textarea class="field" id="hD" maxlength="300"></textarea></div>
    <div class="row"><label for="hDue">Kab tak dena hai</label><input class="field" id="hDue" type="date" value="${addDays(t, 1)}" min="${t}"></div>
    <div class="err" id="hE" role="alert"></div>
    <div class="sheet-actions"><button class="btn ghost" id="hNo">Band karo</button><button class="btn" id="hGo">Save karo</button></div>`);
  const q = x => s.el.querySelector(x);
  q('#hNo').onclick = () => s.close();
  q('#hGo').onclick = async () => {
    const title = q('#hT').value.trim(), due = q('#hDue').value;
    if (!title) return q('#hE').textContent = 'Homework likho.';
    if (!due) return q('#hE').textContent = 'Date chuno.';
    q('#hGo').disabled = true;
    try {
      await addHomework({ subject: q('#hS').value, title, details: q('#hD').value.trim() || null, due_date: due });
      s.close(); toast('Homework add ho gaya ✓'); emitChange();
