...
export function renderProfile(view) {
  ...
  view.innerHTML = `
    ...
    <div class="stats">
      <div class="stat"><b>${mine.length}</b><small>My uploads</small></div>
      <div class="stat"><b>${state.favs.size}</b><small>Favourites</small></div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px">
      <button class="btn ghost" id="pName">✏️ Change name</button>
  ...
  if (!mine.length) grid.innerHTML = `<div class="empty"><h3>No uploads yet</h3><p>Upload your notebook pages to help everyone.</p><a class="btn" href="#/">Upload</a></div>`;
  ...
  view.querySelector('#pName').onclick = async () => {
    const name = await ask({ title: 'Change name', ok: 'Save', input: { label: 'New name', value: state.profile.full_name, required: true, max: 40 } });
    if (!name) return;
    const { error } = await sb.rpc('update_my_name', { p_name: name });
    if (error) return toast(error.message || friendlyError(error), 'err');
    state.profile.full_name = name; toast('Name updated ✓'); emitChange();
  };
}
